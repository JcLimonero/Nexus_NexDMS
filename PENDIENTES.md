...
- [x] **Informe de revisión (puntos de seguridad)** — `mechanic-checklist/informe-revision-pdf.service.ts` + `GET /service-orders/:id/safety-checklist/pdf` (+ `POST .../safety-checklist/email`). Reporte que se entrega al cliente con el **semáforo de cada punto revisado** (llantas, balatas, frenos, niveles…), tomado del catálogo del tenant (`mechanic_checklist_items`) y su evaluación (`mechanic_safety_checklists`: BUENO/OK · REGULAR · MALO/REEMPLAZAR/FALLA). Incluye **resumen** (en buen estado / a vigilar / requiere atención), observaciones por punto y firmas. Botones **"Informe de revisión"** y **"Enviar informe"** en el detalle de la OS.
  - **Captura en el portal**: componente `taller/ordenes-servicio/revision-seguridad` embebido en el detalle de la OS — el mecánico marca el semáforo (Bueno/Regular/Malo/Reemplazar) y la nota por punto, y guarda (`POST .../safety-checklist`). Incluye **mini-CRUD del catálogo** ("Administrar puntos": alta con clave+nombre, baja con chips) vía `GET/POST/DELETE /mechanic-checklist/items[/:id]`.
...
- [x] **Evidencia (fotos/videos) en garantías** — tabla `warranty_evidence` (migración `1792800000000`), `warranties/warranty-evidence.service.ts` + endpoints `GET/POST/DELETE /warranties/:id/evidence[/:evidenceId]` (subida multipart a B2/S3, límite 100 MB, foto o video). Se sube al crear y durante el trámite (OPEN/IN_PROGRESS); una vez cerrada queda de solo lectura. Galería en el detalle de la garantía (fotos con vista previa, videos con reproductor) para **autorizar/rechazar/resolver** con la evidencia a la vista. La **carta de garantía** incrusta hasta 6 **miniaturas de fotos** (el video solo en pantalla). Archivos servidos por URL firmada.
  - ⚠️ Dato de demo: las 3 garantías del tenant Total One se sembraron sin folio (SQL directo); se rellenaron con el generador de códigos → `TDMGA00000001-03`. Si se re-siembra la BD, corregir el seed para que inserten folio.
