        ...
        @if (modoDemo) {
          <div class="demo">
            <div class="demo-titulo">
              {{ clienteNombre() ? "Cuentas de " + clienteNombre() : "Técnicos de demostración" }}
            </div>
...
export class LoginPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Las cuentas del panel pasan a ser las de este cliente (técnico/admin).
    this.auth.demoUsers(slug).subscribe({
      next: (us) => {
        const relevantes = us.filter(
          (u) => u.roles.includes("MECHANIC") || u.roles.includes("ADMIN"),
        );
