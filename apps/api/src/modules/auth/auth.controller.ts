...
/** Fija la cookie de sesión salvo que el cliente pida no hacerlo (monitor). */
function fijarSesionSiCorresponde(res: Response, accessToken: string): void {
  if (!optaPorNoCookie(res.req)) ponerCookiesSesion(res, accessToken);
}
...
export class AuthController {
  ...
  async login(
    ...
    if ("accessToken" in r && r.accessToken) {
      fijarSesionSiCorresponde(res, r.accessToken);
    }
    return r;
  }
  ...
  async refresh(
    ...
    if (r?.accessToken) fijarSesionSiCorresponde(res, r.accessToken);
    return r;
  }
